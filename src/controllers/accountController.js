...
const register = async (req, res) => {
    ...
    try {
        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            ...
            const [rows] = await connection.query('SELECT * FROM users WHERE `phone` = ?', [username]);
            if (rows.length == 0) {
                const sql = "INSERT INTO users (id_user, phone, name_user, password, money, code, invite, veri, ip_address, status, time,plain_password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                let values = [id_user, username, name_user, md5(pwd), 0, code, invitecode1, 1, ip, 1, time,pwd];
                await connection.execute(sql, values);
                console.log("insert into point_list");
                ...
                if (check_ip.length <= 3) {
                    ...
                    await connection.execute(sql, [id_user, username, name_user, md5(pwd), 0, code, invitecode1, ctv, 1, ip, 1, time]);
                    await connection.execute('INSERT INTO point_list SET phone = ?', [username]);
                    return res.status(200).json({
                        message: 'Register Success',
                        status: true
